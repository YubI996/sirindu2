...
@section('item') Data @endsection
@section('item-active') Jenis Vaksin @endsection
...
<style>
    ...
    .st-card .btn-group .btn.btn-sm.btn-success { background: transparent; color: #059669; }
    ...
    .st-modal .modal-header { background: linear-gradient(135deg, var(--st-primary), #059669); color: #fff; border: none; padding: 18px 24px; }
...
<div class="md-page">
    <div class="md-header">
        <div class="md-header__left">
            ...
            <div>
                <h1 class="md-header__title">Master Data Jenis Vaksin</h1>
                <p class="md-header__subtitle">Kelola data referensi jenis vaksin untuk imunisasi anak</p>
            </div>
        </div>
        <div>
    ...
    <div class="st-card">
        ...
        <div class="table-responsive" style="padding: 0;">
            <table id="vaksinTable" class="table" style="width:100%; margin-bottom: 0;">
                <thead>
                    <tr>
                        <th>Kode</th>
                        <th>Nama</th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="formModalTitle">Tambah Jenis Vaksin</h5>
            ...
            <div class="modal-body">
                <form id="vaksinForm">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="st-form-group">
                                <label for="form_kode">Kode Vaksin *</label>
                                <input type="text" id="form_kode" name="kode" placeholder="cth: BCG, DPT1">
                                <div class="text-danger" id="error_kode"></div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="st-form-group">
                                <label for="form_nama">Nama Vaksin *</label>
                                <input type="text" id="form_nama" name="nama" placeholder="cth: Bacillus Calmette-Guerin">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="st-form-group">
                                <label for="form_usia_min">Usia Min (hari)</label>
                                <input type="number" id="form_usia_min" name="usia_pemberian_min" min="0" placeholder="0">
                                <div class="text-danger" id="error_usia_pemberian_min"></div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="st-form-group">
                                <label for="form_usia_max">Usia Max (hari)</label>
                                <input type="number" id="form_usia_max" name="usia_pemberian_max" min="0" placeholder="0">
                                <div class="text-danger" id="error_usia_pemberian_max"></div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="st-form-group">
                                <label for="form_interval">Interval (hari)</label>
                                <input type="number" id="form_interval" name="interval_hari" min="0" placeholder="0">
                                <div class="text-danger" id="error_interval_hari"></div>
                            </div>
                        </div>
                        <div class="col-md-8">
                            <div class="st-form-group">
                                <label for="form_keterangan">Keterangan</label>
                                <textarea id="form_keterangan" name="keterangan" rows="2" placeholder="Keterangan tambahan (opsional)"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="st-form-check">
                        <input type="checkbox" id="form_aktif" name="aktif" checked>
                        <label for="form_aktif">Aktif</label>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn st-btn st-btn-outline-secondary" data-dismiss="modal" style="background:#fff; color:var(--st-text-muted); border-color:var(--st-border);">Batal</button>
...
<div class="modal fade st-modal" id="deleteModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Apakah Anda yakin ingin menghapus jenis vaksin <strong id="deleteVaksinName"></strong>?</p>
                <p style="color:#64748b; font-weight:500; margin-top:8px; font-size:0.85rem;">
                    <span class="material-symbols-outlined" style="font-size:16px; vertical-align:middle;">info</span>
                    Jika vaksin masih digunakan oleh data imunisasi, data akan di-soft-delete dan dapat di-restore.
                </p>
            </div>
            <div class="modal-footer">
...
$(document).ready(function() {
    var table = $('#vaksinTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.masterdata.vaksin.getData") }}',
        columns: [
            { data: 'kode', name: 'kode' },
            { data: 'nama', name: 'nama' },
            ...
            { data: 'interval_hari', name: 'interval_hari', render: function(d) { return d !== null ? d : '-'; } },
            { data: 'status_badge', name: 'aktif' },
    ...
    $('#btnTambah').on('click', function() {
        resetForm();
        $('#formModalTitle').text('Tambah Jenis Vaksin');
    ...
    $(document).on('click', '.btn-edit', function() {
        var id = $(this).data('id');
        resetForm();
        $('#formModalTitle').text('Edit Jenis Vaksin');

        // Get row data from DataTable
        var rowData = table.row($(this).closest('tr')).data();
        $('#form_id').val(rowData.id);
        $('#form_kode').val(rowData.kode);
        $('#form_nama').val(rowData.nama);
        $('#form_kategori').val(rowData.kategori);
        $('#form_usia_min').val(rowData.usia_pemberian_min);
        $('#form_usia_max').val(rowData.usia_pemberian_max);
        $('#form_interval').val(rowData.interval_hari);
        $('#form_keterangan').val(rowData.keterangan);
        $('#form_aktif').prop('checked', rowData.aktif == 1);
        $('#formModal').modal('show');
    });
    ...
    $('#btnSimpan').on('click', function() {
        ...
        var id = $('#form_id').val();
        var url = id
            ? '{{ route("admin.masterdata.vaksin.update", ":id") }}'.replace(':id', id)
            : '{{ route("admin.masterdata.vaksin.store") }}';
        ...
        $.ajax({
            ...
            data: {
                ...
                usia_pemberian_min: $('#form_usia_min').val() || null,
                usia_pemberian_max: $('#form_usia_max').val() || null,
                interval_hari: $('#form_interval').val() || null,
                keterangan: $('#form_keterangan').val(),
                aktif: $('#form_aktif').is(':checked') ? 1 : 0,
            },
    ...
    $(document).on('click', '.btn-toggle', function() {
        var id = $(this).data('id');
        $.ajax({
            url: '{{ route("admin.masterdata.vaksin.toggleStatus", ":id") }}'.replace(':id', id),
            type: 'PATCH',
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        var rowData = table.row($(this).closest('tr')).data();
        $('#deleteVaksinName').text(rowData.nama);
    ...
    $('#confirmDelete').on('click', function() {
        if (!deleteId) return;
        $.ajax({
            url: '{{ route("admin.masterdata.vaksin.destroy", ":id") }}'.replace(':id', deleteId),
            type: 'DELETE',
            data: { _token: '{{ csrf_token() }}' },
            success: function(response) {
                $('#deleteModal').modal('hide');
                var icon = response.soft_deleted ? 'info' : 'success';
                Swal.fire({ icon: icon, title: 'Berhasil', text: response.message, timer: 3000 });
                table.draw();
            },
            error: function(xhr) {
                $('#deleteModal').modal('hide');
    ...
    $(document).on('click', '.btn-restore', function() {
        var id = $(this).data('id');
        Swal.fire({
            title: 'Restore vaksin?',
            text: 'Data vaksin akan dikembalikan ke daftar aktif.',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.masterdata.vaksin.restore", ":id") }}'.replace(':id', id),
                    type: 'PATCH',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(response) {
                        Swal.fire({ icon: 'success', title: 'Berhasil', text: response.message, timer: 2000 });
